...
def total_revenue(data: dict, financial_index: int):
    ...
    """
    print("enterd 2")
    financial_data_entry = data["financials"][financial_index]

    net_revenue = float(financial_data_entry["pnl"]["lineItems"]["net_revenue"])
    return net_revenue
...
def total_borrowing(data: dict, financial_index: int) -> float:
    ...
    long_term_borrowings = financial_entry["bs"]["liabilities"]["long_term_borrowings"]
    short_term_borrowings = financial_entry["bs"]["liabilities"][
    ...
    ratio = total_borrowings / revenue
    return ratio
...
def iscr(data: dict, financial_index):
    ...
    profit_before_interest_and_tax = financial_entry["pnl"]["lineItems"][
        "profit_before_interest_and_tax"
    ]
    depreciation = financial_entry["pnl"]["lineItems"]["depreciation"]
    interest_expense = financial_entry["pnl"]["lineItems"]["interest"]
